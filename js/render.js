...
(function () {
    ...
    // projects
    var pl = $('#projects-list'); if (pl) { pl.innerHTML = ''; S.projects.forEach(function (p, i) { var a = el('a', 'p-row reveal'); a.href = p.href; a.target = '_blank'; a.rel = 'noopener'; a.style.transitionDelay = (i * 0.07) + 's'; a.innerHTML = '<span class="p-num">' + p.n + '</span><div><div class="p-top"><h3>' + p.title + '</h3><span class="p-badge ' + p.tone + '">' + p.badge + '</span></div><p class="p-desc">' + p.desc + '</p><div class="p-tags">' + p.tags.map(function (t) { return '<span>' + t + '</span>'; }).join('') + '</div></div><div style="display:flex; align-items:center; gap:24px;"><img src="assets/project/' + p.n + '.svg" style="width:64px; height:64px; border-radius:4px; opacity:0.85; transition:0.3s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.85" alt="Project QR"><span class="p-arrow">→</span></div>'; pl.appendChild(a); }); }
